...
pv_num = 12
stop_ply = 5
expected_fen_num = int(2 * (pv_num ** stop_ply) * 0.9)
pruning_count = 0
...
def move_forward(engine, ply):
    ...
    if len(fen_books) % 100 == 0:
        print(f"current fen number: {len(fen_books)}, expected total fen number(approximately): {expected_fen_num}")
...
def main():
    ...
    send_command(engine, "setoption name Threads value " + str(thread_num))
